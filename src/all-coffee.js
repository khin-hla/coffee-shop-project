...
badgeContainer.addEventListener('click', (event) => {
  ...
  const filteredCoffees = coffees.filter(item => item.ingredients.includes(selectedIngredient));
...
function addCardToContainer(coffee) {
    ...
    articleEl.innerHTML = `
    <figure class="h-52"><img src=${coffee.image} alt=${coffee.title} loading="lazy" /></figure>
    <div class="card-body">
      <h2 class="card-title">
        ${coffee.title}
